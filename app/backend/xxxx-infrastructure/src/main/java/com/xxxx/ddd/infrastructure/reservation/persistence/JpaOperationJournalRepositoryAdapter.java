...
public class JpaOperationJournalRepositoryAdapter implements OperationJournalRepository {
    ...
    public boolean transition(
        ...
        int updated = entityManager.createNativeQuery(
                        "UPDATE inventory_operation_journal "
                                ...
                                + "result_stock_after = :resultStockAfter, lease_owner = NULL, lease_until = NULL "
    ...
    public List<JournalEntry> claimRecoverable(String workerId, int limit, Duration lease) {
        ...
        entityManager.createNativeQuery(
                        "UPDATE inventory_operation_journal "
                                ...
                                + "WHERE state IN ('RECEIVED', 'REDIS_APPLIED', 'COMPENSATION_PENDING', "
    ...
    private static String selectColumns() {
        return "SELECT operation_id, reservation_id, demo_actor_id, idempotency_key_hash, "
    ...
    private static JournalEntry toJournalEntry(Object[] row) {
        return new JournalEntry(
                ...
                java.util.UUID.fromString(String.valueOf(row[2])),
                PersistenceValueSupport.hex(row[3]),
                PersistenceValueSupport.hex(row[4]),
                ((Number) row[5]).longValue(),
                ((Number) row[6]).intValue(),
                ((Number) row[7]).longValue(),
                JournalState.valueOf(String.valueOf(row[8])),
