import {
  ArrowRight,
  CheckCircle2,
  Flame,
  ShieldCheck,
  Swords,
  Ticket,
  Zap,
} from "lucide-react";
import Link from "next/link";

import { eventStatusLabels, eventSummaries, getFeaturedEvent } from "@/lib/events";
import { formatCurrency } from "@/lib/utils";

import { Badge } from "@/components/ui/badge";
import { Button } from "@/components/ui/button";

export default function Home() {
  const featuredEvent = getFeaturedEvent();
  const secondaryEvents = eventSummaries.filter(
    (event) => event.ticketItemId !== featuredEvent.ticketItemId,
  );

  return (
    <main className="mx-auto flex w-full max-w-[1200px] flex-1 flex-col px-4 py-12 sm:px-6 lg:px-8">
      {/* 1. Hero Section */}
      <section className="grid gap-10 pb-14 lg:grid-cols-[1.05fr_0.95fr] lg:items-center">
        <div className="max-w-3xl">
          <div className="flex items-center gap-2">
            <Badge variant="outline" className="border-[#242424] bg-black text-white text-xs font-semibold px-2.5 py-0.5">
              <Zap className="mr-1 h-3 w-3 text-amber-400" />
              Flash-Sale Concurrency Engine
            </Badge>
            <span className="text-xs text-[#898989]">Spring Boot 3.3 · Redis Cluster · MySQL · Kafka</span>
          </div>
          <h1 className="mt-4 font-display text-5xl font-semibold leading-[1.1] text-[#242424] sm:text-6xl">
            Prove stock correctness under extreme load.
          </h1>
          <p className="mt-5 max-w-2xl text-base leading-7 text-[#898989]">
            Interactive concurrency lab comparing atomic Redis Lua pre-deduction, transactional outbox dual-writes, 
            and scheduled self-healing reconciliation. Prevent overselling and observe zero drift in real-time.
          </p>
          <div className="mt-8 flex flex-wrap gap-3">
            <Button asChild size="lg" className="bg-[#242424] text-white hover:bg-black">
              <Link href="/admin/strategy-arena">
                <Swords className="mr-2 h-4 w-4 text-amber-400" />
                Launch Strategy Battle Arena
              </Link>
            </Button>
            <Button asChild size="lg" variant="secondary">
              <Link href="/admin/control-desk">
                Operator Control Desk
                <ArrowRight className="ml-2 h-4 w-4" />
              </Link>
            </Button>
          </div>
        </div>

        {/* Featured Drop Card */}
        <div className="rounded-xl bg-white p-5 shadow-[rgba(19,19,22,0.7)_0_1px_5px_-4px,rgba(34,42,53,0.08)_0_0_0_1px,rgba(34,42,53,0.05)_0_4px_8px_0]">
          <div className="rounded-lg bg-[#f7f7f7] p-5">
            <div className="flex items-start justify-between gap-4">
              <div>
                <p className="text-xs font-semibold uppercase tracking-[0.08em] text-[#898989]">
                  {featuredEvent.eyebrow}
                </p>
                <h2 className="mt-3 font-display text-3xl font-semibold leading-tight text-[#242424]">
                  {featuredEvent.title}
                </h2>
              </div>
              <Badge className="bg-emerald-600 text-white font-bold">{eventStatusLabels[featuredEvent.status]}</Badge>
            </div>
            <div className="mt-8 grid gap-3 sm:grid-cols-3">
              <TrustItem icon={Ticket} label="Stock Pool" value={featuredEvent.stockLabel} />
              <TrustItem icon={Flame} label="Contention" value={featuredEvent.concurrencyLevel || "Extreme"} />
              <TrustItem
                icon={Zap}
                label="Flash Price"
                value={formatCurrency(featuredEvent.priceFlash)}
              />
            </div>
          </div>
          <div className="mt-4 flex items-center justify-between gap-4">
            <div className="text-xs leading-6 text-[#898989]">
              Fixture ID {featuredEvent.ticketItemId} · {featuredEvent.venue} · {featuredEvent.city}
            </div>
            <Button asChild size="sm" className="bg-[#242424] text-white">
              <Link href={`/events/${featuredEvent.ticketItemId}`}>Live X-Ray Probe</Link>
            </Button>
          </div>
        </div>
      </section>

      {/* 2. Key Architecture Proof Pillars */}
      <section className="grid gap-4 border-t border-black/[0.06] py-10 md:grid-cols-3">
        <ProofItem
          icon={ShieldCheck}
          label="Zero Oversell Guarantee"
          value="0 oversold in safe runs via atomic Redis Lua & DB fencing."
        />
        <ProofItem
          icon={Zap}
          label="Sub-Millisecond Gate"
          value="Rejects 95%+ excess requests in <1ms without touching DB."
        />
        <ProofItem
          icon={CheckCircle2}
          label="Self-Healing Consistency"
          value="Zero drift after dual-fault compensation & 30s reconciler."
        />
      </section>

      {/* 3. Multi-Scenario Drop Board */}
      <section className="pb-10">
        <div className="mb-5 flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
          <div>
            <p className="text-sm font-semibold uppercase tracking-[0.08em] text-[#898989]">
              Multi-Category Scenarios
            </p>
            <h2 className="mt-2 font-display text-3xl font-semibold leading-tight text-[#242424]">
              High-Contention Scenario Fixtures
            </h2>
          </div>
          <Button asChild variant="secondary">
            <Link href="/events">Explore All Fixtures</Link>
          </Button>
        </div>

        <div className="grid gap-4 md:grid-cols-3">
          {secondaryEvents.map((event) => (
            <article
              key={event.ticketItemId}
              className="flex flex-col justify-between rounded-xl bg-white p-5 shadow-[rgba(19,19,22,0.7)_0_1px_5px_-4px,rgba(34,42,53,0.08)_0_0_0_1px,rgba(34,42,53,0.05)_0_4px_8px_0] transition-all hover:border-black"
            >
              <div>
                <div className="flex items-center justify-between">
                  <Badge variant="outline" className="text-xs font-semibold">{event.category}</Badge>
                  <span className="font-mono text-xs font-semibold text-[#898989]">
                    {event.stockLabel}
                  </span>
                </div>
                <h3 className="mt-4 font-display text-xl font-semibold leading-tight text-[#242424]">
                  {event.title}
                </h3>
                <p className="mt-2 text-xs leading-relaxed text-[#666666]">
                  {event.description}
                </p>
                <div className="mt-4 text-base font-bold text-[#242424]">
                  {formatCurrency(event.priceFlash)}{" "}
                  <span className="text-xs font-normal text-[#898989] line-through">
                    {formatCurrency(event.priceOriginal)}
                  </span>
                </div>
              </div>

              <div className="mt-5 border-t border-[#f0f0f0] pt-3">
                <Button asChild variant="secondary" size="sm" className="w-full text-xs font-semibold">
                  <Link href={`/events/${event.ticketItemId}`}>
                    Launch Scenario Probe
                    <ArrowRight className="ml-1.5 h-3 w-3" />
                  </Link>
                </Button>
              </div>
            </article>
          ))}
        </div>
      </section>
    </main>
  );
}

function TrustItem({
  icon: Icon,
  label,
  value,
}: {
  icon: React.ComponentType<{ className?: string }>;
  label: string;
  value: string;
}) {
  return (
    <div className="rounded-lg bg-white p-4 shadow-[rgba(34,42,53,0.10)_0_0_0_1px]">
      <Icon className="h-4 w-4 text-[#242424]" />
      <div className="mt-4 text-xs font-semibold uppercase tracking-[0.08em] text-[#898989]">
        {label}
      </div>
      <div className="mt-2 text-sm font-semibold leading-5 text-[#242424]">{value}</div>
    </div>
  );
}

function ProofItem({
  icon: Icon,
  label,
  value,
}: {
  icon: React.ComponentType<{ className?: string }>;
  label: string;
  value: string;
}) {
  return (
    <div className="rounded-xl border border-[#ebebeb] bg-[#fafafa] p-4">
      <Icon className="h-5 w-5 text-[#242424]" />
      <div className="mt-3 text-xs font-semibold uppercase tracking-[0.08em] text-[#898989]">
        {label}
      </div>
      <div className="mt-1.5 text-sm font-semibold text-[#242424] leading-snug">{value}</div>
    </div>
  );
}
